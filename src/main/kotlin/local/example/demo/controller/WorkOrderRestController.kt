...
class WorkOrderRestController internal constructor(
    ...
    internal fun read(@PathVariable id: Int?): Resource<WorkOrder> {
        val workOrder = workOrderRepository.findById(id!!)
                .orElseThrow { WorkOrderNotFoundException(id) }
        return workOrderResourceAssembler.toResource(workOrder)
    }

    @GetMapping
    @Throws(URISyntaxException::class)
    internal fun readAll(): Resources<Resource<WorkOrder>> {
